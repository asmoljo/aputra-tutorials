...
func prepStmt(db *sql.DB, c Config, op string) (stmt *sql.Stmt, err error) {
	...
	} else if c.Test.Db == "sqlite3" && op == "create_cart" {
		return db.Prepare("INSERT INTO cart(customer_id, total) VALUES (?, ?) RETURNING id")
	} else if c.Test.Db == "pgx" && op == "add_cart_item" {
		return db.Prepare("INSERT INTO cart_item(cart_id, product_id, quantity) VALUES ($1, $2, $3)")
	} else if c.Test.Db == "sqlite3" && op == "add_cart_item" {
	...
	} else if c.Test.Db == "sqlite3" && op == "update_cart_total" {
	...
	} else if c.Test.Db == "sqlite3" && op == "create_order" {
		return db.Prepare("INSERT INTO `order`(customer_id, total) VALUES (?, ?) RETURNING id")
	...
	} else if c.Test.Db == "sqlite3" && op == "add_order_item" {
	...
	} else if c.Test.Db == "sqlite3" && op == "update_product_quantity" {
	...
	} else if c.Test.Db == "sqlite3" && op == "delete_cart_item" {
	...
	} else if c.Test.Db == "sqlite3" && op == "delete_cart" {
	...
	} else if c.Test.Db == "sqlite3" && op == "test2" {
...
func (c *Cart) createCart(stmt *sql.Stmt, m *metrics) (err error) {
	now := time.Now()
	defer func() {
		if err == nil {
...
func (o *Order) createOrder(stmt *sql.Stmt, m *metrics) (err error) {
	now := time.Now()
	defer func() {
		if err == nil {
