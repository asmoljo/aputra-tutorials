...
func (ms *MyServer) dbConnect(ctx context.Context) {
	uri := fmt.Sprintf("mongodb://%s:27017", ms.config.DbConfig.Host)

	opts := options.Client().SetMaxPoolSize(ms.config.DbConfig.MaxConnections)
	...
	ms.db = client.Database(ms.config.DbConfig.Database)
}
